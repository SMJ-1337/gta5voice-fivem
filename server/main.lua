...
local function removePlayerFromPool(target, id, poolData)
  ...
  -- update the player data
  gta5voice.PlayerPool[source][poolData.value] = nil;
...
AddEventHandler('playerDropped', function()
  ...
  if RadioId then
    removePlayerFromPool(tonumber(source), RadioId, { name = 'RadioPool', value = 'radioId' });
  end

  -- if the player was in a call, then remove him
  if CallId then
    removePlayerFromPool(tonumber(source), CallId, { name = 'CallPool', value = 'callId' });
  end
end);